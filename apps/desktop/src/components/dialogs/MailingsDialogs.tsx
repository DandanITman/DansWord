 ...
 *
 * The live previews are not decoration. Word's Address Block and Greeting Line
...
/**
 * Word's Envelopes and Labels dialog, both tabs in one window.
 *
 * Keeping them together matters: the two share the address you just typed, and
 * Word lets you switch tabs without losing it. Two separate dialogs would make
...
/**
 * Word's Mail Merge Recipients dialog.
 *
 ...
 * Word makes that four clicks deep.
 */
...
/** The columns Word's New Address List starts with. */
...
/** Word's New Address List: build a data source without leaving the app. */
...
/** Word's Insert Merge Field dialog: the full column list, with Insert repeatable. */
  ...
  return (
    <FormDialog title="Insert Merge Field" testId="insert-merge-field-dialog" onClose={onClose}>
      ...
      <div className="dialog-actions">
        ...
        {/* Insert without closing, as Word does, so several fields can go in
            one after another. */}
        <button
          className="icon-btn primary"
          disabled={!selected}
 ...
 * are chosen by rule, which is exactly what Word's separate dialogs amount to.
 */
...
/** Word's Merge to New Document / Merge to Printer: pick the record range. */
...
/**
 * Word's six-step wizard, as a task pane.
 *
