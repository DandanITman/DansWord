...
/**
 * The picture node view: Word's selection frame, handles and drag behaviour.
 *
 * Two things here matter for the picture to feel like Word's:
 *
 ...
 *    margin, and to the baseline grid, drawing the same alignment guides Word
 *    shows. Without them, "centred" was a pixel-hunt that never quite landed.
 */
export function ImageBlockView({ node, updateAttributes, selected, editor, getPos }: NodeViewProps) {
  ...
  // Delete and arrow-key nudging, exactly as Word treats a selected picture.
  useEffect(() => {
    if (!selected) return;
  ...
  const startResize = (event: React.MouseEvent, handle: HandleId) => {
    ...
    const onMove = (moveEvent: MouseEvent) => {
      ...
      // Shift inverts the aspect lock while dragging, as Word's modifier does.
  ...
  const startRotate = (event: React.MouseEvent) => {
    ...
    const onMove = (moveEvent: MouseEvent) => {
      ...
      // Shift constrains to 15° steps, as Word does.
  ...
  return (
    <NodeViewWrapper
      ...
      onMouseDown={(event: React.MouseEvent) => {
        // A floating picture is dragged by its body, as in Word. An inline one
