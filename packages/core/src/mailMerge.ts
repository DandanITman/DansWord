/**
 * Mail merge: the data model and every rule Word's Mailings tab can express.
 *
 ...
 *
 * Names follow Word's own: "recipient" for a row, "field" for a column, and the
 * standard address-field names Match Fields maps onto. Anyone who has used
 * Word's tab should recognise the vocabulary.
 */
 ...
 *
 * A directory is the one type that does not: Word's directory merge exists
...
export interface MergeRecipient {
   ...
   *
   * Word's Merge Record # reports the row's place in the data source, not its
 ...
 * trailing empty columns, and Word reads those files. A blank or duplicated
...
/**
 * The address fields Word's Match Fields dialog knows about.
 *
...
/**
 * Guess the mapping from the column headers, the way Word does on attaching a
...
/** The name layouts Word's Insert Address Block offers, in its own order. */
...
export interface AddressBlockOptions {
  ...
  /** Word's "Never / Always / Only include when different from" country rule. */
 ...
 * 2 and no company, and Word does not print gaps for them.
 */
export function buildAddressBlock(
  ...
  // "City, State  Postal Code" - Word's spacing, with each part optional.
...
/** The name layouts Word's Greeting Line dialog offers. */
...
export interface GreetingLineOptions {
  ...
  punctuation: string;
  /** Used when the row has no name at all - Word's "Dear Sir or Madam,". */
...
/** Word's own Rules menu, in Word's order. */
 ...
 * document as the same inline node - exactly as Word stores them all as fields.
 */
...
export interface MergeContext {
  recipient: MergeRecipient | null;
  /** Word's Merge Record #: the row's place in the data source. */
  recordNumber: number;
  /** Word's Merge Sequence #: how many records this merge has emitted. */
...
/** How an unmerged field shows in the document: Word's «Field» chevrons. */
  ...
  switch (attrs.kind) {
    ...
    case 'rule':
      switch (attrs.rule) {
        ...
        case 'fillIn':
          // Word asks once per merge and reuses the answer; the prompt's answer
    ...
    default: {
       ...
       *
       * This is how Ask and Set Bookmark become useful. Word pairs them with a
       ...
       * already met keeps one concept where Word has two, and Check for Errors
...
/**
 * Word's Check for Errors, minus the parts that need a printer.
 *
...
/** The label stocks Word lists by default, sized as their vendors specify. */
