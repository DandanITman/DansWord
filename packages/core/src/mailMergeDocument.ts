...
export function collectMergePrompts(doc: unknown): Array<{
  ...
  for (const attrs of collectMergeFields(doc)) {
    ...
    // prompt text is the key - matching Word, where two identically worded
...
/** Text with newlines becomes runs separated by hard breaks, as Word's does. */
 ...
 *
 * Word's Next Record advances mid-document, which is how one label sheet holds
...
export function executeMerge(
  ...
  // Word's behaviour, and it beats handing back an empty file.
...
/**
 * The label sheet Word's Update Labels produces: an address block in the first
